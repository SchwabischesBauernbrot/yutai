...
const RegexParser = struct {
    ...
    fn raiseError(comptime parser: *RegexParser, comptime fmt: []const u8, args: anytype) void {
        ...
        inline for (args) |arg| {
            if (@TypeOf(arg) == ?u21) {
                if (arg) |cp| {
                    tuple = tuple ++ .{ctUtf8EncodeChar(cp)};
                } else {
                    tuple = tuple ++ .{"null"};
                }
            } else if (@TypeOf(arg) == u21) {
                tuple = tuple ++ .{ctUtf8EncodeChar(arg)};
            } else {
                tuple = tuple ++ .{arg};
            }
        }
        ...
        const error_slice = std.fmt.bufPrint(&error_buf, "error: {}: " ++ fmt, .{parser.iterator.i - 1} ++ tuple) catch unreachable;
    ...
    fn parseAsciiIdent(comptime parser: *RegexParser) []const u8 {
        ...
        if (c != '_' and !std.ascii.isAlphabetic(@as(u8, @truncate(c)))) {
        ...
        readChars: while (true) {
            ...
            if (c > 127 or (c != '_' and !std.ascii.isAlphanumeric(@as(u8, @truncate(c)))))
                break :readChars;
    ...
    fn parseBracketsRule(comptime parser: *RegexParser) ?Brackets.Rule {
        ...
        if (parser.consumeChar('-')) {
            ...
            const second_char = if (parser.consumeChar('\\')) block: {
                ...
                parser.raiseError("Invalid character '{}' after escape \\", .{parser.peek()});
            } else parser.consumeNotOneOf(special_brackets) orelse parser.raiseError("Expected a valid character after - in bracket rule", .{});
    ...
    fn charClassMinLen(comptime _: u21, comptime _: Encoding) usize {
        return 1;
    }
            ...
            pub usingnamespace if (capture_len != 0)
                struct {
                    pub fn capture(self: Self, comptime name: []const u8) ?[]const CharT {
                        inline for (capture_names, 0..) |maybe_name, curr_idx| {
