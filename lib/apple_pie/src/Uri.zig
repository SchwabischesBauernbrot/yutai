...
pub fn decode(allocator: std.mem.Allocator, value: []const u8) DecodeError![]const u8 {
    ...
    while (i < value.len) : (i += 1) {
        switch (value[i]) {
            '%' => {
                perc_counter += 1;
                if (i + 2 > value.len or !std.ascii.isAlphanumeric(value[i + 1]) or !std.ascii.isAlphanumeric(value[i + 2])) {
...
fn parseSchema(uri: *Uri, buffer: []const u8) error{InvalidCharacter}!void {
    if (!std.ascii.isAlphabetic(buffer[0])) return error.InvalidCharacter;
...
fn isLs32(char: u8) bool {
    return std.ascii.isAlphanumeric(char) or char == ':' or char == '.';
}
