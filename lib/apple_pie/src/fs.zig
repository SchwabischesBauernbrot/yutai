...
pub fn serve(ctx: void, response: *Response, request: Request) ServeError!void {
    ...
    if (std.mem.startsWith(u8, path, "..")) return ServeError.NotFound;

    const file = try dir.openFile(path, .{});
    defer file.close();

    const value = if (immutable)
        "max-age=604800, immutable"
    else
        "no-cache";
    try response.headers.put("Cache-Control", value);
    errdefer _ = response.headers.swapRemove("Cache-Control");

    try serveFile(response, request.path(), file);
}
...
pub fn serveFile(
    ...
    switch (response.buffered_writer.unbuffered_writer.context.*) {
        .secure => {
            var fifo = FIFO.init();
            defer fifo.deinit();

            try fifo.pump(file.reader(), response.buffered_writer.writer());
            try response.buffered_writer.flush();
        },
        .plain => |plain| {
            const nh = plain.handle;
            const fh = file.handle;
            var r: u64 = len;
            while (r > 0) {
                r -= try std.os.sendfile(nh, fh, len - r, r, &.{}, &.{}, 0);
            }
        },
    }
}
