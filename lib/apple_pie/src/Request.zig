...
const max_buffer_size = blk: {
    ...
    break :blk @min(given, 1024 * 1024 * 16); // max stack size
};
...
pub const Form = struct {
    fields: Uri.KeyValueMap,
    files: Uri.KeyValueMap,
...
pub fn form(self: Request, gpa: Allocator) !Form {
    var map = Uri.KeyValueMap{};
    errdefer map.deinit(gpa);

    var files = Uri.KeyValueMap{};
    ...
    while (try it.next(gpa)) |field| {
        ...
        if (field.filename) |filename| {
            try map.map.put(gpa, field.key, try gpa.dupe(u8, filename));
            try files.map.put(gpa, filename, field.value);
        } else {
...
const FormIterator = struct {
    ...
    batch_itr_opt: ?SplitIterator = null,
    ...
    const Field = struct {
        ...
        pub fn deinit(self: Field, gpa: Allocator) void {
            gpa.free(self.key);
            gpa.free(self.value);
            if (self.filename) |filename| gpa.free(filename);
        }
    };

    fn parseChunk(gpa: Allocator, raw: []const u8) !?Field {
        const fd = "; filename=\"";
        const fdl = fd.len;
        const data = std.mem.trim(u8, raw, "-");
        var field: Field = undefined;

        const key_start_opt = std.mem.indexOf(u8, data, "name=");
        if (key_start_opt == null) return null;
        const key_start = key_start_opt.? + "name=\"".len;
        const key_end = std.mem.indexOfScalarPos(u8, data, key_start, '"') orelse
            return FormError.BadForm;

        var i = key_end + 1;
        const tmp = i + fdl;
        field.filename = if (data.len > tmp and std.mem.eql(u8, data[i..tmp], fd)) blk: {
            const fname_start = tmp;
            const fname_end = std.mem.indexOfScalarPos(u8, data, fname_start, '"') orelse
                return FormError.BadForm;
            i = fname_end + 1;
            break :blk try gpa.dupe(u8, data[fname_start..fname_end]);
        } else null;
        errdefer if (field.filename) |filename| gpa.free(filename);

        const value_start = (std.mem.indexOfPos(u8, data, i, "\r\n\r\n") orelse
            return FormError.BadForm) + "\r\n\r\n".len;
    ...
    pub fn next(self: *FormIterator, gpa: Allocator) !?Field {
        ...
        switch (self.form_type) {
            ...
            .multipart => |boundary_name| {
                if (self.batch_itr_opt == null) {
                    self.batch_itr_opt = std.mem.split(
                        u8,
                        self.form_data,
                        boundary_name,
                    );
                }
                const batch_itr = &self.batch_itr_opt.?;
                var field: ?Field = null;
                while (field == null) {
                    const batch = batch_itr.next();
                    if (batch == null) break;
                    field = try parseChunk(gpa, batch.?);
                }
                return field;
            },
            .url_encoded => {
...
pub fn parse(
    ...
    reader: anytype,
    buffer: []u8,
) (ParseError || Stream.ReadError)!Request {
    return Request{
        .arena = gpa,
...
fn parseContext(
    ...
    reader: anytype,
    buffer: []u8,
) (ParseError || @TypeOf(reader).Error)!Context {
    var ctx: Context = .{
        .method = .get,
        .uri = Uri.empty,
    ...
    var parser = Parser(@TypeOf(reader)).init(gpa, options, buffer, reader);
...
fn Parser(comptime ReaderType: anytype) type {
    return struct {
        const Self = @This();
        ...
        fn parseStatus(self: *Self) Error!?Event {
            self.state = .header;
            const line_opt = try self.reader.readUntilDelimiterOrEof(self.buffer, '\n');
            const line = line_opt orelse return ParseError.EndOfStream;
        ...
        fn parseBody(self: *Self) Error!?Event {
            ...
            if (self.content_length != 0) {
                return if (self.ignoreContent()) blk: {
                    try self.reader.skipBytes(self.content_length, .{});
                    break :blk error.ContentTooLarge;
                } else blk: {
                    const raw_body = try self.gpa.alloc(u8, self.content_length);
                    try self.reader.readNoEof(raw_body);
                    break :blk Event{ .body = raw_body };
                };
            }
            ...
            return if (self.ignoreContent())
                error.ContentTooLarge
            else
                Event{ .body = try body_list.toOwnedSlice() };
        }
        ...
        fn ignoreContent(self: *const @This()) bool {
            return self.content_length > self.options.max_length;
        }
    };
}
